        ...
        Ok(program) => {
            match &program.program_unit {
                fortran_ast::ProgramUnit::Subroutine(sub) => {
                    println!("\n// FORTRAN 77 → Modern Type Mapping (Generated by Ferrum)");
                    ...
                    println!("pub struct FortranComputer {{");
                    ...
                    println!("    }}");
                    println!();
                    println!("    pub fn compute<F>(");
                    println!("        &mut self,");
                    println!("        y: &[f64],           // Variables");
                    println!("        f: F,                // Function evaluator");
                    println!("        output_matrix: &mut [f64], // Output matrix");
                    println!("        options: &ComputeOptions,");
                    println!("    ) -> Result<ComputeResult, ComputeError>");
                    println!("    where");
...
fn analyze_parameter(name: &str) -> (&'static str, &'static str, &'static str) {
    match name {
        ...
        "MATRIX" => ("&mut [f64]", "np.ndarray", "[out] Output matrix"),
        "LDM" => ("usize", "int", "[in] Leading dimension"),
